...
| GET | `/api/providers/:id` | PUBLIC | Get provider (accepts User ID or Provider ID, returns extended user data) |
...
| GET | `/api/monitoring/system-health` | PUBLIC | Comprehensive system health check (database, external APIs, app settings, system metrics) |
...
7. **Provider Lookup**: Provider endpoints (`/api/providers/:id` and `/api/marketplace/providers/:id`) accept either User ID or Provider ID. The system queries the Users collection first, then retrieves the provider profile, returning extended data with both User and Provider information.
8. **System Health**: The `/api/monitoring/system-health` endpoint provides comprehensive health status including database connectivity, external API status, app settings, and system metrics. Returns HTTP 200 for healthy systems, 503 for degraded systems.
