...
import { Navigation, MapPin, Grid3x3, List, Loader2, Sparkles, X, Plus, BarChart3 } from "lucide-react";
...
export default function MarketplacePage() {
  ...
  // Get user roles and roleView state
  ...
  // Job filters state (for provider view)
  ...
  // Fetch max price once on initial load (without filters) to set the price range for services
  useEffect(() => {
    if (isClientView) {
  ...
  // Fetch max salary for jobs (provider view)
  useEffect(() => {
    if (isProviderView) {
      const fetchMaxSalary = async () => {
        try {
          const response = await fetch(
            `${API_BASE_URL}/api/jobs?limit=1&sortBy=salary&sortOrder=desc`,
            createAuthFetchOptions()
          );
          if (response.ok) {
            const data = await response.json();
            ...
            if (jobs.length > 0) {
              ...
              if (salary > 0) {
                ...
                setSalaryRange((prev) => prev[1] === 200000 ? [0, calculatedMax] : prev);
              }
            }
          }
        } catch (error) {
  ...
  // Reset page to 1 when filters change (services)
  useEffect(() => {
    if (isClientView) {
      setCurrentPage(1);
    }
  }, [categoryKey, subcategory, location, locationCoordinates, radius, priceRange, minRating, isAvailable, sortBy, sortOrder, groupByCategory, isClientView]);

  // Reset page to 1 when job filters change (jobs)
  useEffect(() => {
    if (isProviderView) {
      setCurrentPage(1);
    }
  }, [jobSearch, jobCategory, jobSubcategory, jobType, experienceLevel, jobLocation, isRemote, salaryRange, company, featured, sortBy, sortOrder, isProviderView]);
  ...
  // Build query parameters for jobs fetch (provider view)
  ...
  // Fetch services with filters applied via query parameters (client view)
  const { featuredServices, services, loading: loadingServices, pagination: servicesPagination } = useMarketplaceServices(servicesParams);

  // Fetch jobs with filters applied via query parameters (provider view)
  const { jobs, loading: loadingJobs, pagination: jobsPagination } = useJobs(jobsParams);
  ...
  // Get role-specific greeting and description
  const getGreeting = () => {
    if (isClientView) {
      return `Hi ${getUserName()}, what service do you need today?`;
    } else if (isProviderView) {
      return `Hi ${getUserName()}, welcome to your marketplace`;
    }
    return `Hi ${getUserName()}, welcome to the marketplace`;
  };

  const getDescription = () => {
    if (isClientView) {
      return "Discover and book trusted service providers in your area";
    } else if (isProviderView) {
      return "Browse and discover job opportunities, or manage your job postings";
    }
    return "Browse services and manage your listings";
  };
  ...
  // Active filters for services (client view)
  const hasActiveServiceFilters = useMemo(() => {
  ...
  // Active filters for jobs (provider view)
  const hasActiveJobFilters = useMemo(() => {
  ...
  return (
    <>
      <div className="bg-gray-50 min-h-screen">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
            ...
            <div className="flex items-start justify-between mb-6 lg:mb-8">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm sm:text-base text-gray-600">{getDescription()}</p>
              </div>
        ...
        {/* Main Content */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          {/* AI Natural Language Search - Show primarily for clients */}
          {isClientView && (
            <div className="mb-6">
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            {/* Left Sidebar - Filters (Services for client, Jobs for provider) */}
            {isClientView ? (
              <FilterSidebar
              ...
              <JobFilterSidebar
                ...
                onSubcategoryChange={setJobSubcategory}
                jobType={jobType}
                ...
                hasActiveFilters={hasActiveJobFilters}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mb-6 bg-white rounded-xl p-4 lg:p-5 shadow-sm border border-gray-200">
                <div className={`flex flex-col sm:flex-row items-start sm:items-center gap-4 ${isClientView ? 'justify-between' : 'justify-end'}`}>
                  {/* Left Side - Location Detection (More prominent for clients) */}
                  {isClientView && (
                  ...
                  {/* Provider View - Compact location (optional) */}
                  {isProviderView && locationCoordinates && (
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      <MapPin className="w-4 h-4 text-gray-400" />
                      <span>{(radius / 1000).toFixed(1)} km radius</span>
                      <button
                        onClick={() => {
                          setLocationCoordinates(null);
                          setLocation("");
                  ...
                  <div className={`flex flex-wrap items-center gap-2 sm:gap-3 ${isClientView ? 'w-full sm:w-auto' : 'w-full'}`}>
                    {/* Sort By */}
                    <select
                      value={sortBy}
                    ...
                    {/* Group By Category Toggle - Only for services */}
                    {isClientView && (
              ...
              {/* AI Service Recommendations - Only for clients */}
              {isClientView && showAIFeatures.recommendations && (
                <div className="mb-6">
                  <LazyAIServiceRecommendations
                    location={location}
                    lat={locationCoordinates?.lat}
              ...
              {/* AI Service Matcher - Only for clients */}
              {isClientView && showAIFeatures.serviceMatcher && (
                <div className="mb-6">
                  <LazyAIServiceMatcher
                    location={location}
                    lat={locationCoordinates?.lat}
              ...
              {/* Provider View - Info about browsing jobs */}
              {isProviderView && (
                <div className="mb-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <p className="text-sm text-gray-700">
                    <span className="font-medium">Browse job opportunities</span> Find the perfect role or post your own job listings to attract talent.
                  </p>
                </div>
              )}

              {/* Service Listings (Client View) */}
              {isClientView && (
                <ServiceGrid 
      ...
      {/* Floating AI Price Estimator - Show primarily for clients */}
      {isClientView && showAIFeatures.priceEstimator && (
        <div className="fixed bottom-6 left-6 z-[9999] max-w-sm w-96">
          <LazyAIPriceEstimator />
        </div>
      )}
      
      {/* Floating AI Features Toggle - Show primarily for clients */}
      {isClientView && (
      ...
      {/* Floating Preferred Feature Selector - Right Side - Show primarily for clients */}
      {isClientView && preferredFeatureSelectorOpen && (
