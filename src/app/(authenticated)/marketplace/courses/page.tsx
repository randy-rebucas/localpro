...
export default function MarketplaceCoursesPage() {
    ...
    // Categories state is set but reserved for future filtering features
    ...
    const normalizeCourse = useCallback((course: Partial<Course> & Record<string, unknown>): Course => {
        ...
        if (pricingValue && typeof pricingValue === 'object') {
            ...
            const pricingObj = pricingValue as Record<string, unknown>;
            normalizedPricing = {
        ...
        const instructorObj = course.instructor as Record<string, unknown> | undefined;
        const normalizedInstructor = typeof course.instructor === 'string'
            ? { id: course.instructor }
            : {
                _id: (instructorObj?._id as string | undefined) || (instructorObj?.id as string | undefined),
                id: (instructorObj?.id as string | undefined) || (instructorObj?._id as string | undefined),
                name: instructorObj?.name as string | undefined,
                firstName: instructorObj?.firstName as string | undefined,
                lastName: instructorObj?.lastName as string | undefined,
                avatar: instructorObj?.avatar as string | undefined,
                bio: instructorObj?.bio as string | undefined,
                rating: instructorObj?.rating as number | undefined,
                reviewCount: instructorObj?.reviewCount as number | undefined,
                verified: instructorObj?.verified as boolean | undefined
            };
        
        // Handle rating
        const ratingObj = course.rating as Record<string, unknown> | undefined;
        const averageRating = (course.averageRating as number | undefined) ?? (ratingObj?.average as number | undefined);
        const ratingCount = (course.ratingCount as number | undefined) ?? (course.reviewsCount as number | undefined) ?? (ratingObj?.count as number | undefined);
    ...
    const fetchCourses = useCallback(async (isRefresh = false) => {
        ...
        try {
            ...
            if (!isAuthenticated()) {
                logger.error("User not authenticated - no api-token cookie found");
                logger.debug("Available cookies", { cookieCount: document.cookie.split(';').filter(c => c.trim()).length });
            ...
            logger.debug("Courses data", { data });
