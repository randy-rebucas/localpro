...
export default function CreateRentalPage() {
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      // Build params for rental description generation using rental-specific endpoint
      const params = {
  ...
  const handleSave = async (_status: 'draft' | 'active') => {
    ...
    try {
      if (!getApiToken()) {
        throw new Error('Please log in to create a rental');
      }
      ...
      if (response.ok) {
        ...
        // Structure 1: { success: true, data: { _id: "...", ... } }
        // Structure 2: { success: true, data: { id: "...", ... } }
        // Structure 3: { id: "...", ... } (direct)
        const rentalId = responseData.data?._id || 
        ...
        if (rentalId) {
          ...
          logger.error('Error creating rental', new Error('Rental created but no ID returned'), { responseData });
          // Still redirect to rentals list if ID is missing
          router.push('/rentals');
        }
      } else {
        const errorData = await response.json();
        logger.error('Error creating rental', new Error('Rental creation failed'), { errorData, status: response.status });
      }
    } catch (error) {
      logger.error('Error creating rental', error instanceof Error ? error : new Error(String(error)));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-white mb-1">Create New Rental</h1>
            <p className="text-sm text-slate-400">List your equipment, vehicle, or space for rent</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => setPreviewMode(!previewMode)}
              className="flex items-center gap-2 px-4 py-2 bg-slate-800 border-2 border-slate-700 text-slate-300 rounded-lg hover:bg-slate-700 transition-all shadow-sm hover:shadow-md font-medium"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-4 py-2 bg-slate-800 border-2 border-slate-700 text-slate-300 rounded-lg hover:bg-slate-700 transition-all shadow-sm hover:shadow-md font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gradient-to-br from-white to-gray-50/50 rounded-xl border-2 border-gray-200 shadow-lg p-6 backdrop-blur-sm">
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-500">You can add specific availability schedules after creating the rental.</p>
              </div>
            </div>
