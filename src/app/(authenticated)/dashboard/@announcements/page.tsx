...
export default function AnnouncementsPage() {
  ...
  const handleDismiss = async (id: string) => {
    try {
      ...
      // Check endpoint pattern - announcementsAcknowledge may be /api/announcements/[id]/acknowledge
      const endpoint = API_ENDPOINTS.announcementsAcknowledge.includes('[id]') 
        ? API_ENDPOINTS.announcementsAcknowledge.replace('[id]', id)
        : `${API_ENDPOINTS.announcementsAcknowledge}/${id}/acknowledge`;
      const url = `${API_BASE_URL}${endpoint}`;
      await fetch(url, createAuthFetchOptions({ method: 'POST' }));
    } catch (error) {
      logger.error('Error acknowledging announcement', error instanceof Error ? error : new Error(String(error)));
  ...
  // Debug logging
  logger.debug('Announcements state', { 
