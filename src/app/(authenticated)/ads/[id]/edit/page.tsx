  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.error('Error updating ad', undefined, { errorData, adId: params.id });
      }
    } catch (error) {
      logger.error('Error updating ad', error instanceof Error ? error : new Error(String(error)), { adId: params.id });
    } finally {
      setSaving(false);
    }
  };
