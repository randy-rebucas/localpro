...
export default function UsersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const usersUrl = `${API_BASE_URL}${API_ENDPOINTS.users}${usersQuery ? `?${usersQuery}` : ''}`;
      ...
      const [dataResponse, statsResponse] = await Promise.all([
        ...
        fetch(statsUrl, createAuthFetchOptions({ method: 'GET' })).catch(err => { // fallback
  ...
  const handleDeleteUser = async (userId: string) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        ...
        const endpoint = API_ENDPOINTS.usersDelete.includes('[id]')
          ? API_ENDPOINTS.usersDelete.replace('[id]', userId)
  ...
  const handleSuspendUser = async (userId: string) => {
    try {
      ...
      const endpoint = API_ENDPOINTS.usersStatus.includes('[id]')
        ? API_ENDPOINTS.usersStatus.replace('[id]', userId)
      ...
      logger.error('Error suspending user', err instanceof Error ? err : new Error(String(err)), { userId });
  ...
  const handleActivateUser = async (userId: string) => {
    try {
      ...
      // Use the same pattern as suspend - replace [id] in usersStatus endpoint
      const endpoint = API_ENDPOINTS.usersStatus.includes('[id]')
        ? API_ENDPOINTS.usersStatus.replace('[id]', userId)
        : `${API_ENDPOINTS.users}/${userId}/status`;
      const url = `${API_BASE_URL}${endpoint}`;
      ...
      logger.error('Error activating user', err instanceof Error ? err : new Error(String(err)), { userId });
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="mt-2 sm:mt-0 flex items-center space-x-2">
          ...
          <button
            onClick={() => logger.debug('Create new user')}
