...
function SignInForm() {
  ...
  const sendVerificationCode = async (phone: string) => {
    ...
    if (!API_BASE_URL) {
      logger.error("API_BASE_URL is not configured", new Error('API_BASE_URL is not configured'));
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          ...
          toast.error(errorMsg);
          setErrors({ phone: errorMsg });
          logger.error("Network error details", new Error(errorMsg), {
  ...
  const verifyAndSignIn = async () => {
    ...
    try {
      ...
      logger.error("Verify code error", error instanceof Error ? error : new Error(String(error)), { phoneNumber, code: verificationCode });
  ...
  const onSubmit = async (data: SignInForm) => {
    logger.debug("On submit data", { hasPhone: !!data.phone });
    ...
    if (data.phone) {
      ...
      logger.debug("Formatted phone", { formattedPhone, original: data.phone });
