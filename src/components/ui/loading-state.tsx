...
const getLoadingContent = (type: string, size: string) => {
  ...
  switch (type) {
    case "dashboard":
      return (
        <div className={`${sizeClasses[size as keyof typeof sizeClasses]} bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg p-6 animate-pulse`}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {Array.from({ length: 3 }).map((_, j) => (
                      <div key={j} className="h-3 bg-slate-700 rounded w-1/2"></div>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className={`${sizeClasses[size as keyof typeof sizeClasses]} bg-slate-900/80 border border-slate-800 rounded-2xl shadow-lg p-6 animate-pulse`}>
          ...
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="border-l-4 border-slate-700 bg-slate-800 rounded-lg p-4">
