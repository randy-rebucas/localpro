...
type UserStatus = 'active' | 'inactive' | 'suspended' | 'pending_verification' | 'banned';

type BusinessType = 'individual' | 'small_business' | 'enterprise' | 'franchise';

type BackgroundCheckStatus = 'pending' | 'approved' | 'rejected' | 'not_required';

type AgencyRole = 'owner' | 'admin' | 'manager' | 'supervisor' | 'provider';

type AgencyStatus = 'active' | 'inactive' | 'suspended' | 'pending';

type ReferralSource = 'email' | 'sms' | 'social_media' | 'direct_link' | 'qr_code' | 'app_share';

type ReferralTier = 'bronze' | 'silver' | 'gold' | 'platinum';

type BadgeType = 'verified_provider' | 'top_rated' | 'fast_response' | 'reliable' | 'expert' | 'newcomer';
...
export function UserProfile({ initialProfile }: { initialProfile?: UserProfileData }) {
  ...
  const { roleView } = useRoleView({ userRoles });
